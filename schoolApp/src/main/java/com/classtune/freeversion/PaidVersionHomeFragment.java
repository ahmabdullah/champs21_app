...
public class PaidVersionHomeFragment extends Fragment implements
	...
	private void setUp() {
		...
		for (int i = 0; i < myDiaryArrayText.length; i++) {
			...
			child.setClazzName("com.classtune.schoolapp.fragments."
					+ myDiaryArrayClazz[i]);
	...
	private void loadDiaryItem(DrawerChildMenuDiary item) {
		...
		try {
			...
			if (className.equals("com.classtune.schoolapp.fragments.home")) {
				((HomePageFreeVersion) getActivity()).loadHome();
				return;
			}
			if (className
					.equals("com.classtune.schoolapp.fragments.SchoolFeedFragment")) {
